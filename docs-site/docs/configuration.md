---
title: Configuration
description: Configure profiles, budgets, API, MCP, LLM, tenancy, and logs.
---
...
Configuration is the boundary between reusable core behavior and host-specific policy. Domain content belongs here, not in core services.
...
The root config file is config/evidence-risk-review.php; profile files live under config/evidence-risk-review/profiles when published.
...
```env
EVIDENCE_RISK_REVIEW_API_ENABLED=false
EVIDENCE_RISK_REVIEW_MCP_ENABLED=false
EVIDENCE_RISK_REVIEW_LLM_ENABLED=false
EVIDENCE_RISK_REVIEW_LOG_STORE=null
```
