---
title: Default OFF
description: Keep external capabilities disabled until the host opts in.
---
...
Safe package installation means no routes, no MCP exposure, no model calls, and no persistence by default.
...
Default-OFF applies to api.enabled, mcp.enabled, llm.enabled, and log store configuration.
