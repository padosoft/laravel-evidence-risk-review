---
title: Tenancy
description: Keep persisted reviews scoped to the host tenant context.
---
...
The package accepts tenant identifiers without depending on a tenancy package or host model.
...
Bind TenantResolver in multi-tenant hosts; clients cannot spoof server-resolved tenancy.
...
```php
$this->app->singleton(TenantResolver::class, CurrentTenantResolver::class);
```
