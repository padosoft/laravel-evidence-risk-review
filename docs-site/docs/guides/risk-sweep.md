---
title: Risk Sweep
description: Run configured checks and reduce findings into verdicts.
---
...
Risk sweep searches for unsupported strength, over-generalization, special populations, contraindications, boundary conditions, and red flags.
...
Checks emit structured findings; verdict precedence is remove > flag > soften > keep.
...
```php
$findings = $sweep->sweep($artifact, $profile, $meter);
```
