...
description: Bind the single LLM contract only when semantic review is needed.
---
...
The package intentionally has no OpenAI, Anthropic, or vendor SDK dependency. Hosts own the transport and credentials.
...
Implement EvidenceReviewerLlmContract and set llm.enabled=true.
...
```php
$this->app->bind(EvidenceReviewerLlmContract::class, MyReviewer::class);
```
