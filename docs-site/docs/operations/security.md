---
title: Security
description: Keep auth, tenancy, secrets, and model calls host-owned.
---
...
The package supplies hooks and contracts, not a security perimeter. Hosts own HTTP middleware, tenant context, and provider credentials.
...
No API keys or model provider SDKs are required by the package.
...
```text
Package: contracts and hooks
Host: auth, tenants, credentials
```
