...
description: Model domain policy without hardcoding domain logic.
---
...
Domain profiles keep medical, legal, finance, and engineering policy in config so the core package remains reusable.
...
A profile declares enabled checks, minimum tiers, keyword lists, and verdict policy.
...
```php
'enabled_checks' => ['evidence_strength', 'red_flag'],
```
