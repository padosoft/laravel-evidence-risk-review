---
title: Verdicts
description: Understand keep, soften, flag, and remove.
---
...
Verdicts are operational recommendations, not moral labels. They tell an adapter what it may safely do next.
...
RiskVerdict is ordered by severity so multiple findings reduce deterministically.
...
```text
remove > flag_for_human_review > soften > keep
```
