...
description: Formalize when a cited source supports a claim scope.
---
...
Evidence boundaries separate citation existence from citation adequacy. A source may exist and still be too weak, too narrow, or too context-specific for a claim.
...
The boundary model links ClaimRef.sourceIds to ranked SourceRef objects and profile thresholds.
...
```text
claim scope <= source scope AND source rank >= profile threshold
```
