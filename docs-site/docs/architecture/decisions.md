...
description: Read the core ADRs behind the package.
---
...
ADR pages capture decisions that future contributors should not rediscover by accident.
...
Each decision follows problem, decision, consequence, trade-off.
...
```text
Problem -> Decision -> Consequences -> Trade-off
```
