---
title: Adapters
description: Keep PHP, CLI, HTTP, and MCP surfaces thin.
---
...
Adapters are allowed to validate and shape input, but not to duplicate risk rules.
...
The same payload shape should work across reviewArray, HTTP, and MCP.
...
```text
Controller -> Request DTO -> ReviewEngine -> Json response
```
