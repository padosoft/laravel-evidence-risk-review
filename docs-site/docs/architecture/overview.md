...
description: Map the one-core, many-adapters architecture.
---
...
Architecture overview prevents drift: public surfaces marshal data, while the core engine decides behavior.
...
ReviewEngine depends on profile registry, labeler, sweep, log store, and clock.
...
```php
$result = app(ReviewEngine::class)->review($artifact, $options);
```
