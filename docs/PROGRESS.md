...
- Copilot follow-up on PR #21 found the raw-component guard scanned fenced/inline code and could false-positive legitimate examples. Updated `docs-site/scripts/check-no-raw-html.mjs` to skip fenced code blocks and inline code while still rejecting raw PascalCase component tags in prose.
- Copilot follow-up on PR #21 found the homepage still called the pending custom domain the official docs URL. Updated `docs-site/docs/index.md` to mirror the README pending DNS/deploy wording. Kept sidebar labels `Concetti & Teoria` and `Architettura` because they are explicitly required by the user prompt.
- Copilot/Codex review follow-up on PR #21: fixed docs references for the actual default profile env var (`EVIDENCE_RISK_REVIEW_PROFILE`), actual default HTTP prefix (`evidence-risk-review`), and made docmd asset paths root-relative in config before rebuilding.
- Copilot review on PR #21 found that the README called the docs URL official while DNS/deploy was not live. Updated README wording to mark the Cloudflare custom domain as pending DNS/deploy verification until the external Cloudflare step is completed.
...
- Subtask PR #13 was merged into `macro/w6-mcp-surface` as merge commit `c9cecba`.
- W6 macro local gates passed without per-W AI review, per the temporary final-deep-review override:
  - `composer validate --strict --no-interaction --no-ansi`
  - `vendor/bin/pint --test`
  - `vendor/bin/phpstan analyse --memory-limit=512M --no-progress`
